...
FROM employees e
WHERE e.employee_id NOT IN (SELECT a.employee_id FROM assignments a);
...
SELECT e.employee_id,
       e.first_name || ' ' || e.last_name AS full_name,
       e.department,
       ROUND(COALESCE(avg_assign.avg_hours, 0), 2) AS avg_hours_worked,
...
) AS avg_assign ON e.employee_id = avg_assign.employee_id
...
SELECT ds.department,
       ...
       GREATEST(ds.avg_salary, ds.max_salary) AS greatest_of_avg_and_max,
       LEAST(ds.avg_salary, ds.max_salary) AS least_of_avg_and_max
FROM dept_stats ds
